/*
In the previous module, we learned about how values have owners. We transfer ownership of a value by moving ownership from one variable to another. 
...
The variable greeting_reference was of type string reference (&String). 
Since we only borrowed greeting and didn't move ownership, greeting could still be used after we created greeting_reference.

*/

/*
References in functions
This example isn't too interesting since we're not actually using greeting_reference for anything.
Let's take a look at how we use references in functions.
...
If you take a closer look at the compiler error, you'll see a hint about changing our reference to be mutable by changing the type parameter from &String to &mut String. 
We also need to declare our original value as mutable:
*/
fn main() {
...
Now we get to the real core of Rust's memory management story. 
Immutable and mutable references differ in one other way that has radical effects on how we build our Rust programs.
When we borrow a value of any type T, the following rules apply:
