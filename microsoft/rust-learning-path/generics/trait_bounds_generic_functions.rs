...
/*
Here, we specify the trait name and the impl keyword. We specify with these values instead of using a concrete type for the value parameter. 
The value parameter accepts all types that use the defined trait. 
Because the function doesn't know anything about the concrete type it will receive, it can only use the methods available by the trait bounds of the anonymous type parameter.
